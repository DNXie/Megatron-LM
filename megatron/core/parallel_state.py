...
def get_nccl_options(pg_name, nccl_comm_cfgs):
    ...
    if pg_name in nccl_comm_cfgs:
        ...
        if "cga_cluster_size" in nccl_comm_cfgs[pg_name]:
            nccl_options.config.cga_cluster_size = nccl_comm_cfgs[pg_name][
                "cga_cluster_size"
            ]
...
def create_hierarchical_groups(
    ...
    assert rank not in ranks or len(hierarchical_groups) == len(
        hierarchical_group_sizes
    )
    assert rank not in ranks or len(hierarchical_groups_gloo) == len(
        hierarchical_group_sizes
    )
...
def default_embedding_ranks(pp_ranks, split_rank=None):
    ...
    if len(pp_ranks) == 1:
        return [pp_ranks[0]]
    elif split_rank is not None and pp_ranks[split_rank] not in (
        pp_ranks[0],
        pp_ranks[-1],
    ):
...
def initialize_model_parallel(
    ...
    if (
        ...
        or encoder_pipeline_model_parallel_size != 0
    ):
        warnings.warn(
            ...
            for i in range(num_distributed_optimizer_instances):
                ...
                if rank in intra_partial_dp_ranks_with_cp:
                    _INTRA_PARTIAL_DATA_PARALLEL_GROUP_WITH_CP = (
                        intra_partial_dp_group_with_cp
                    )
    ...
    assert (
        _EXPERT_DATA_PARALLEL_GROUP is None
    ), "Expert data group is already initialized"
    global _EXPERT_DATA_PARALLEL_GROUP_GLOO
    assert (
        _EXPERT_DATA_PARALLEL_GROUP_GLOO is None
...
def get_model_parallel_src_rank():
    ...
    assert (
        _MODEL_PARALLEL_GLOBAL_RANKS is not None
    ), "Model parallel group is not initialized"
...
def get_expert_data_parallel_group(
    check_initialized=True, partial_expert_data_parallel=False
):
